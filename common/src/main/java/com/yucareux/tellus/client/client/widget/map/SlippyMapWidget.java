...
import com.yucareux.tellus.client.client.widget.map.component.MapComponent;
...
public class SlippyMapWidget extends AbstractWidget {
	...
	protected void renderWidget(@NonNull GuiGraphics graphics, int mouseX, int mouseY, float delta) {
		...
		cascadedTiles.sort(Comparator.comparingInt(SlippyMapTilePos::zoom));
	...
	private void renderTile(
		...
		if (location != null) {
			int deltaZoom = cameraZoom - pos.zoom();
			double zoomScale = Math.pow(2.0, deltaZoom);
			int size = Mth.floor(SlippyMap.TILE_SIZE * zoomScale);
			int renderX = (pos.x() << deltaZoom) * SlippyMap.TILE_SIZE - cameraX;
			int renderY = (pos.y() << deltaZoom) * SlippyMap.TILE_SIZE - cameraY;
